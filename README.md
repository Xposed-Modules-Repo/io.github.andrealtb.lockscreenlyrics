...
- 翻译按钮锁屏态门禁与即时刷新：仅在锁屏状态下激活注入，解锁进入桌面瞬间自动平滑撤出；修复内部重绑机制复合 Key 传递，灭屏与解锁瞬间即时刷新，无需切歌或暂停。
- 收紧翻译按钮门禁：仅在播放器确实由 Provider 服务且当前曲目包含翻译时才允许展示翻译按钮，未适配播放器或无翻译歌曲严格保持 100% 原生。
